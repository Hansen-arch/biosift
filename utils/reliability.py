...
def compute_reliability_score(df):
    ...
    # 4. missing date penalty
    if "eventDate" in df.columns:
        scores -= df["eventDate"].isna().astype(int) * 10

    # 5. GBIF issues penalty
...
def get_data_fitness(score, summary, temporal_stats):
    ...
    suitability.append({
        ...
        if spatial_ok else
        "Too many low-precision coordinates for fine-scale work (<100m)"
    })
...
def generate_methods_text(species, df, clean_df, summary, score):
    ...
    if dup > 0:
        removed_parts.append(f"duplicates (n={dup})")

    zc = summary.get("zero_coords", {}).get("count", 0)
    if zc > 0:
        removed_parts.append(f"zero-coordinate records (n={zc})")

    old = summary.get("old_record", {}).get("count", 0)
    if old > 0:
        removed_parts.append(f"pre-1900 records (n={old})")

    mc = summary.get("missing_coords", {}).get("count", 0)
    if mc > 0:
        removed_parts.append(f"records missing coordinates (n={mc})")

    lp = summary.get("low_precision", {}).get("count", 0)
    if lp > 0:
        removed_parts.append(f"low-precision coordinate records (n={lp})")
    ...
    text = (
        ...
        f"scientificName='{species}', hasCoordinate=TRUE, limit={retrieved:,}. "
        ...
        f"All data are available under their respective licences via GBIF.org."
    )
